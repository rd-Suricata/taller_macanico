...
// Consulta SQL base
$sql = "SELECT p.people_id, p.name, p.surname, p.email, p.phone_number, p.document_type, p.document_number,
...
<div class="container mx-auto mt-8 p-4">
    ...
    <form method="GET" action="" class="mb-6 p-4 bg-gray-100 rounded-lg shadow-md max-w-4xl mx-auto">
        ...
        <div class="mt-4 flex justify-center">
            ...
            <a href="./registro.php" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 mx-4 rounded focus:shadow-outline">
              Agregar
            </a>
        </div>
    </form>
    ...
    <table class="w-full bg-white shadow-md rounded">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo "<td class='py-3 px-6 text-left'>" . htmlspecialchars($row["job_title"]) . "</td>";
                echo "<td class='py-3 px-6 text-left'>" . htmlspecialchars($row["salary"]) . "</td>";
                ...
                echo "<td class='py-3 px-6 text-left'>
                        <a href='./view.php?id=" . $row['people_id'] . "' class='text-green-500 hover:text-blue-700'>
